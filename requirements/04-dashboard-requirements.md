...
## 33. AI 활용 Feedback 톤 및 임시 테스트 생성 (2026-09-09 보완)

- 공통 GlassPanel, primary 보라색, 중립 surface, Material Symbols 아이콘을 사용한다. 이모지·노란 점수 축하 효과·랜덤 채점 연출을 제거한다.
- 정식 Feedback 생성은 프로젝트 `COMPLETED`에만 활성화한다. Task 진행률 100%만으로 해제하지 않으며 기존 자동 집계 결과가 있으면 즉시 조회한다. 없는 경우 `POST /api/utilization`으로 리포트와 지표 기반 SYSTEM 제안을 생성한다.
- 개발 확인용 `테스트 중` 토글을 임시 제공한다. 기본 OFF이며 프로젝트 전환 시 초기화한다. ON이면 완료 전에도 `POST /api/utilization/preview`로 현재 지표의 잠정 점수·제안을 계산한다.
- 테스트 결과는 PREVIEW로 표시하고 DB에 저장하지 않는다. 프로젝트 상태·Task·QA·Git·정식 리포트 및 비교 기준은 변경하지 않는다. OFF 전환 시 임시 결과를 버리고 정식 완료 조건을 다시 적용한다.
- 임시 기능은 `TEMP UF_TEST_PREVIEW` 주석으로 표시한다. 제거 시 UI 토글, preview 클라이언트/타입, preview 라우트/서비스 및 해당 테스트를 함께 정리한다. 정상 `POST /api/utilization`의 완료 검증은 유지한다.

### 34. Feedback 생성 진행 표시 및 요약 패널 압축
- 생성 버튼을 누르면 원형 내부가 아래에서 차오르며 0~100% 표시 후 결과로 전환한다.
- 표시 준비 애니메이션은 실제 활용 점수와 구분한다. API 응답 전에는 최대 90%에서 대기하며, 성공 후 100%를 잠시 보여준다. 실패하면 오류와 재시도 버튼을 제공한다.
- 프로젝트 전환 시 진행 중 표시를 정리하며, 모션 감소 설정에서는 애니메이션을 생략한다.
- 흐름도 옆 Token Usage/현재 계획은 세로로 늘리지 않고 컴팩트한 프로젝트 요약으로 제공한다.
- 개요·작업·품질·결과물 탭은 실제 snapshot 요약을 표시하고 기존 상세 패널 바로가기를 유지한다.

### 35. 프로젝트 요약 전체 펼침 (34항 탭 구성 대체)
- 프로젝트 요약 탭을 제거하고 Token Usage·현재 계획·작업·품질·결과물을 동시에 표시한다.
- 데스크톱에서 Attention Center와 요약을 합친 오른쪽 열이 Development Flow 높이를 채운다. 각 항목은 간결한 요약과 상세 바로가기를 유지한다.
- 에이전트 미배정 시 역할 카드를 유지하며, 에이전트 수에 따라 흐름도 높이를 축소하지 않는다. DRAFT/AGENT_MATCHING/READY 상태에서는 기존 에이전트 매칭 화면을 표시한다.

### 36. 흐름도 미배정 역할 비활성화
- 미배정 역할은 회색 점선 카드와 배정 후 활성화 안내로 표시하고 상세 클릭을 비활성화한다.
- 양쪽 역할에 에이전트가 배정된 연결만 컬러 화살표와 흐르는 애니메이션을 표시한다. 미배정 역할 연결은 회색 정적 점선으로 남긴다.
- REMOVED 에이전트는 배정에 포함하지 않으며 snapshot의 배정 변경에 따라 자동 갱신한다.
