...
### 3. 데모 자동 반복 시연 — `demo_replay`

준비된 **Neobank Super App** 프로젝트로 계획 검토 → 실행 승인 → 역할별 병렬 개발 → 품질 검증 → 마일스톤 결과 승인 → 프로젝트 완료를 자동으로 반복 시연하는 데모 실행용 에이전트입니다. 스케줄은 백엔드 서버가 실행하므로 CLI를 종료해도 계속 진행됩니다. 프로젝트 최상위 폴더에서 다음 명령으로 실행합니다.

```bash
cd backend
venv/bin/python seed_demo.py             # Neobank Super App 팀·태스크·마일스톤 생성(재실행 시 갱신)
venv/bin/python demo_replay.py start     # 시연 시작 (기본 대상 http://127.0.0.1:8000)
venv/bin/python demo_replay.py status    # 진행 상태 확인
venv/bin/python demo_replay.py stop      # 시연 정지
```

- **http://localhost:5173**을 새로고침하고 프로젝트 선택에서 **Neobank Super App**을 고르면 SSE/상태 조회로 자동 갱신됩니다.
- `--interval`은 각 단계 유지 시간(초, 기본 3), `--hold`는 완료 화면 유지 시간(초, 기본 15), `--url`은 다른 서버 주소를 지정합니다.
- 이 시연에는 `EXECUTION_MODE=demo`, `GIT_MODE=stub`, 값이 비어 있는 `QA_TEST_CMD`가 필요합니다. 실제 모델 호출과 셸 테스트 실행은 발생하지 않습니다.
- 실제 GitHub 커밋·게시로 시연하려면 `backend/.env`에 `DEMO_REPLAY_GIT_MODE=real`을 설정하고 백엔드를 재시작합니다. 자세한 동작·검증은 `backend/README.md`의 **Neobank 자동 반복 시연** 항목을 참고하세요.

### 4. 다섯 역할의 전체 작업 흐름 확인
...
실행 중인 로컬 서버에서 화면의 버튼과 입력만으로 프로젝트 생성부터 완료·피드백까지 수행한 화면 증거는 [screenshots/README.md](screenshots/README.md)에 있습니다. 예제 모델 응답과 모의 게시 설정에서 촬영한 화면이며, 실제 모델의 코드 품질이나 원격 게시 성공의 증거로 사용하지 않습니다. 실제 생성 파일의 검증 경로는 [안정성 개선 근거](aidlc-docs/construction/stability-review.md) 문서의 통합 테스트로 확인합니다.
